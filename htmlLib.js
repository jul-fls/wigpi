...
async function GenerateHTML(classname, date1, $cours_of_the_week) {
    ...
    timeSlots.forEach(timeSlot => {
        ...
        for (let day = 0; day <= 4; day++) {
            ...
            if (courseForSlot && !renderedCourses[`${day}-${courseForSlot.heure_debut}`]) {
                ...
                const $courseCell = $('<td class="shadow-neumorphism rounded-2xl border-[32px] border-transparent text-center"></td>')
    ...
    const screenshotUrl = `http://192.168.1.131:20002/screenshot?url=https://wigpi.flusin.fr/api/courses/get_html/${classname}`;
    try {
