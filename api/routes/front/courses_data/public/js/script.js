...
fetch("https://wigpi.flusin.fr/api/classes/get_json")
  ...
  .then((data) => {
    ...
    const filterClassStatus = document.getElementById("filterClassStatus");

    ["Terminé", "En cours", "Non commencé"].forEach((status) => {
      const optionFilterStatus = document.createElement("option");

      optionFilterStatus.value = status;
      optionFilterStatus.textContent = status;

      filterClassStatus.appendChild(optionFilterStatus);
    });
  })
  .catch((error) => {
  ...
  document.querySelectorAll("#dataContainer > div").forEach((card) => {
    ...
    statusIcons.forEach((statusIcon) => {
      if (status === "Terminé" && statusIcon.innerHTML.includes("Terminé")) {
        ...
        status === "En cours" &&
        statusIcon.innerHTML.includes("En cours")
      ) {
        card.style.display = "block";
      } else if (
        status === "Non commencé" &&
        statusIcon.innerHTML.includes("Non commencé")
      ) {
        card.style.display = "block";
      } else {
        card.style.display = "none";
    ...
    // if (status === "done" && statusIcon.title == "Terminé") {
    //   console.log("card :: ", card);
    //   card.style.display = "block";
    // } else if (
    //   status === "in progress" &&
    //   statusIcon.classList.contains("fa-calendar-day")
    // ) {
    //   console.log("card :: ", card);
    //   card.style.display = "block";
    // } else if (
    //   status === "planned" &&
    //   statusIcon.classList.contains("fa-lock")
    // ) {
    //   card.style.display = "block";
    // } else {
    //   card.style.display = "none";
